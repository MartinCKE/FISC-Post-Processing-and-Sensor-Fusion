...
def profilePlot(files, ace):
	...
	## Bulding meshgrid with bad to optimal values for temp and O2 saturation ##
	temps = np.linspace(7,20, 100)
	o2_sats = np.linspace(60, 100, 100)

	tempMesh, o2Mesh = np.meshgrid(temps, o2_sats)
	#z = (1 - tempMesh / 2. + x ** 5 + y ** 3) * np.exp(-x ** 2 - y ** 2)
	#print()
	test = tempMesh + o2Mesh
	fig, ax = plt.subplots(1)



	c = ["darkred","red","palegreen","green","blue"]
	#v = [0,.15,.4,.5,0.6,.9,1.]
	v = [0, 0.2, 0.6, 0.8, 1]
	l = list(zip(v,c))
	...
	plot = plt.pcolormesh(temps, o2_sats, test, cmap=cmap, shading='flat')
	#plt.imshow(test)
	plt.show()
	quit()
