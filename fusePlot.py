	...
	while(True):
		...
		if ret == True:
			try:
				...
				#print("Try:", str(re.findall('[0-9]{2}:[0-9]{2}:[0-9]{2}(?:.\d{5}\b)?', str(curr_frame_timestamp)))) ## To get HH:MM from filename)
				#print(str(curr_frame_timestamp)[-15:])
				...
				#print("Except:", str(re.findall('[0-9]{2}:[0-9]{2}:[0-9]{2}(?:.\d{5}\b)?', str(curr_frame_timestamp))))
				#print(str(curr_frame_timestamp)[-15:])
				#quit()
				timestamps.append(str(curr_frame_timestamp)[-15:])
			#print("curr:", timestamps[-1])

			#print("timestamps:", timestamps)
			'''
			print("\n\r")
			print("i:", i)
			print("len timsetamps:", len(timestamps))
			print("rxfiles in video:", rxFilesInVideo)
			print("len rxfiles n video:", len(rxFilesInVideo))
			print("rx files timestamps:", rxFiles_timeStamps)
			print("len rx files timestamps:", len(rxFiles_timeStamps))
			print("Current video:", videoFile)
			print("Start time:", startTime, "End time:", endTime)
			print("Current RX file timestamp:", rxFiles_timeStamps[i])
			print("\n\r")
			'''
			...
			if abs(rxFiles_timeStamps[i] - curr_frame_timestamp) < datetime.timedelta(milliseconds=20):
				...
				#genSyncPlot(axs, frame, startTime, rxFilesInVideo[i], rxFiles_timeStamps[i], **kwargs)
